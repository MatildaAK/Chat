defmodule NewchatWeb.RoomLive.FormComponent do
  use NewchatWeb, :live_component
  ...
  def update(%{room: room} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Chatrooms.change_room(room))
     end)}
  end

  @impl true
  def handle_event("validate", %{"room" => room_params}, socket) do
    changeset = Chatrooms.change_room(socket.assigns.room, room_params)
  ...
  defp save_room(socket, :edit, room_params) do
    case Chatrooms.update_room(socket.assigns.room, room_params) do
      {:ok, room} ->
