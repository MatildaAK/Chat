defmodule NewchatWeb.MessageLive.Index do
  ...
  def mount(_params, _session, socket) do
    changeset = Chatmsg.change_message(%Message{})
    user = socket.assigns.current_user

    if connected?(socket) do
      subscribe()
      {:ok, _} = Presence.track(self(), @presence_topic, user.id, %{name: user.username})
      Phoenix.PubSub.subscribe(Newchat.PubSub, @presence_topic)
    end
  ...
  def subscribe() do
    Phoenix.PubSub.subscribe(Newchat.PubSub, "liveview_chat")
  end

  def notify({:ok, message}, event) do
    Phoenix.PubSub.broadcast(Newchat.PubSub, "liveview_chat", {event, message})
  end
